...
public class DashboardDockablePanel extends JPanel implements Dockable {
	...
	private EngineOptions.Builder engineOptionsBuilderHw;
	...
	private void setupBrowser() {
		//Creating Builder per version 10 requirement
		engineOptionsBuilderHw = EngineOptions.newBuilder(RenderingMode.HARDWARE_ACCELERATED);
		...
		if (BrowserUtilities.WEBAPP_DEVELOPMENT_ACTIVATED) {
			...
			browser = BrowserFactory.createEngine(engineOptionsBuilderHw, "Developer Tools").newBrowser();
		} else {
...
//		BrowserUtilities.prepareResourceURLLoading(engineOptionsBuilderHw, true, true, Collections.emptyMap());
