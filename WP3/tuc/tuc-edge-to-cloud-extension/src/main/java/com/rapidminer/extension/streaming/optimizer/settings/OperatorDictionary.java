/**
 * Authors: Ourania Ntouni, Dimitrios Banelas
 * <p>
...
import com.rapidminer.extension.streaming.operator.tuc.FederatedLearning;
import com.rapidminer.extension.streaming.operator.tuc.FederatedLearningTuner;
 ...
 * CREXDATA optimizer. The method {@link #createFromNetwork(String, Network, List)} provides the functionality to create such a
...
public class OperatorDictionary {
    ...
    public static Logger LOGGER = LogService.getRoot();
        ...
        for (Operator operator : operators) {
            ...
            for (OptimizerSite site : network.getSites()) {
                ...
                currentSite.setStaticCost(50 + 10 * random.nextInt(5))
                        ...
                        .setPlatforms(List.of(platforms));
    ...
    private static List<Operator> createAllStreamingOperators() throws OperatorCreationException {
        ...
        result.add(OperatorService.createOperator(FederatedLearningTuner.class));
        result.add(OperatorService.createOperator(FederatedLearning.class));
