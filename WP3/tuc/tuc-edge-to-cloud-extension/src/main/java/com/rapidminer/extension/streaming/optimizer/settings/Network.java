 ...
 * on the sites) for the CREXDATA optimizer. The method {@link #create(String, Map)} CREXDATA the
    ...
    public static Network create(String name,
        ...
        for (Entry<String, List<ConnectionInformationContainerIOObject>> entry : availableSites.entrySet()) {
            ...
            for (ConnectionInformationContainerIOObject connection : entry.getValue()) {
                ...
                LOGGER.info("Connection type: " + connType);

                ConnectionHandler handler = StreamingConnectionHelper.CONNECTION_HANDLER_MAP.get(connType);
                if (handler instanceof RTSAConnectionHandler) {
                    String platformName = connType.replace("admin:", "");
                    String rtsaServerUrl = connConfig.getParameter("rtsa.server_url").getValue();
                    String rtsaHost = rtsaServerUrl.split(":")[0];
                    String rtsaPort = rtsaServerUrl.split(":")[1];

                    platforms.add(new OptimizerPlatform().setPlatformName(platformName)
                            .setConnectionHost(rtsaHost)
                            .setConnectionPort(rtsaPort)
                            .setFramework(platformName));

                    continue;
                }

                // At this point, we know that the connection is a streaming connection
                StreamConnectionHandler streamHandler = (StreamConnectionHandler) handler;

                Properties connectionProperties = streamHandler.buildClusterConfiguration(connConfig);
