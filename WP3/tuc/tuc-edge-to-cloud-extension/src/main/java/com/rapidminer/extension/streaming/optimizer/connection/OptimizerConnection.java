...
public class OptimizerConnection {
	...
	public OptimizerConnection(String host, String port, String fileServerHost, String fileServerPort, String username
		...
		this.logger = LogService.getRoot();
	...
	public String sentOptimizationRequest(String networkJSON, String dictionaryJSON, String requestJSON,
		...
		this.logger.severe("sentOptimizationRequest: Sending optimization request to Optimizer service");
		// If the stompSession was not yet connect, create the connection
		if (stompSession == null || !stompSession.isConnected()){
			this.logger.severe("sentOptimizationRequest: Stomp session not connected, trying to connect");
			try {
		...
		ResponseEntity<String> responseMessage = restTemplate.postForEntity(
		...
		try {
			...
			String requestId = JsonUtil.fromString(responseMessage.getBody(), OptimizerMessage.class).getId();
	...
	public OptimizerResponse waitForOptimizerPlan(int pollingTimeMilliSeconds) throws InterruptedException {
        logger.info("At OptimizerConnection - waitForOptimizerPlan");
	...
	private void connectStompSession(long timeOutSessionConnect) throws ExecutionException,
		...
		this.logger.severe("connectStompSession: Trying to connect to: " + webSocketUrl);
