...
import static com.rapidminer.extension.streaming.connection.crexdata.MaritimeConnectionHandler.PARAMETER_ALLOW_SELF_SIGNED_CERT;
import static com.rapidminer.extension.streaming.connection.crexdata.MaritimeConnectionHandler.PARAMETER_HOST;
import static com.rapidminer.extension.streaming.connection.crexdata.MaritimeConnectionHandler.PARAMETER_PASSWORD;
import static com.rapidminer.extension.streaming.connection.crexdata.MaritimeConnectionHandler.PARAMETER_PORT;
import static com.rapidminer.extension.streaming.connection.crexdata.MaritimeConnectionHandler.PARAMETER_USERNAME;
...
import com.rapidminer.extension.streaming.connection.crexdata.MaritimeConnectionHandler;
import com.rapidminer.extension.streaming.deploy.flink.FlinkRestClient;
import com.rapidminer.extension.streaming.deploy.crexdata.MaritimeRestClient;
...
import com.rapidminer.extension.streaming.deploy.management.db.crexdata.MaritimeStreamingEndpoint;
...
public class DashboardAPIHandler {
	...
	public void openDeployedProcess(String wId, String jId) {
		...
		if (optimizationJob != null && optimizationJob.getType().equals(Type.CrexdataOptimizer)) {
			CrexdataOptimizerStreamingEndpoint endpoint = (CrexdataOptimizerStreamingEndpoint) optimizationJob.getEndpoint();
	...
	private void doOptimizationUpdate(Workflow workflow, String jId, CrexdataOptimizerStreamingEndpoint endpoint) {
		try {
	...
	private Process openProcessAndAdaptToLatestPlan(Workflow workflow, String jId,
                                                    CrexdataOptimizerStreamingEndpoint endpoint, boolean newPlan){
	...
	private void doStopJob(String wId, Job job) {
		...
		switch (job.getType()) {
            ...
            case CrexdataOptimizer:
                CrexdataOptimizerStreamingEndpoint optimizerStreamingEndpoint =
					(CrexdataOptimizerStreamingEndpoint) endpoint;
				try {
					...
					LOGGER.log(Level.SEVERE, "Could not stop CREXDATA Optimizer Request with request-id: " + jobId,
						optimizerStreamingEndpoint.getRequestId());
					status = job.getState();
				}
				break;
	...
	private JobDTO adapt(Job job) {
		...
		if (job.getType() == Type.CrexdataOptimizer){
			optimizerResponseInfo = ((CrexdataOptimizerStreamingEndpoint) job.getEndpoint()).newPlanInfos();
		}
